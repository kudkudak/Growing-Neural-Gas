...
public:
    ...
    UniformGrid(double * origin, int *dim,  int gng_dim, double m_grow_factor = 1.5,  
            double m_density_threshold = 2.0, double m_density_threshold_min = 0.4): 
    m_dist_fnc(0), gng_dim(gng_dim), m_density_threshold(m_density_threshold),
    ...
    double m_grow_factor = 1.5,  
            double m_density_threshold = 2.0, 
            double m_density_threshold_min = 0.4
    ): m_dist_fnc(0), gng_dim(gng_dim) 
    ...
    long int calculate_new_size(double *origin, double *axis, double l) {
        ...
        REP(i, this->gng_dim){ 
            ...
            if(result > UniformGrid::MAX_SIZE) return -1;
        }
        return result;
    }
    ...
    bool check_grow(){
        unsigned long int result = this->calculate_new_size(m_origin, m_axis, m_l/m_grow_factor);
