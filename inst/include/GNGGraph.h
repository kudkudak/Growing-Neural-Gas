	...
	public:
            ...
            */ 
            void serialize(std::string filename){
                ...
                vector<double> S; S.reserve(10000);
                ...
                DBG(7, "GNGGraph::Serializing nodes");
                //Nodes
                for(int i=0;i<g.size();++i){
                   if(existsNode(i)){
                	   ...
                	   std::copy(serialized_node.begin(), serialized_node.end(), std::back_inserter(S));
                   }else{
                	   S.push_back((double)0);
                   } 
                }
                DBG(7, "GNGGraph::Serializing edges");
                //Edges
                for(int i=0;i<g.size();++i){
                   if(existsNode(i)){
                ...
                DBG(7, "GNGGraph::Serializing nextFree");
                //NextFree
                for(int i=0;i<g.size();++i){
                	S.push_back((double)next_free[i]);
                }
                DBG(7, "GNGGraph::Serialize;:writing out");
            ...
            void load(std::string filename){
                ...
                std::ifstream input; input.open(filename.c_str(), ios::in | ios::binary);

                DBG(7, "GNGGraph:: loading "+filename);
